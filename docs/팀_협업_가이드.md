...
```
┌──────────────────────────────────────────────────────────────────────┐
│  팀원 로컬 컴퓨터           GitHub 저장소           Cafe24 서버        │
│                                                                      │
│  [내 브랜치 작업]                                                      │
│  코드 수정/추가    ──push──>  [내 브랜치]                               │
│                                  │                                   │
...
| 단계 | 담당 | 설명 |
|------|------|------|
| 1️⃣ 코드 수정 | 팀원 | 로컬 컴퓨터에서 파일 수정 |
| 2️⃣ Commit & Push | 팀원 | 변경사항을 내 브랜치에 저장 후 GitHub에 올리기 |
| 3️⃣ PR 생성 | 팀원 | GitHub에서 검토 요청 (Reviewer: YoungBin 지정) |
| 4️⃣ 코드 리뷰 | YoungBin | 변경된 코드 검토, 피드백 또는 승인 |
| 5️⃣ Merge | YoungBin | 문제 없으면 main 브랜치에 병합 |
| 6️⃣ 자동 배포 | GitHub Actions | Merge 즉시 Cafe24 서버로 자동 배포 |
...
**주요 책임:**
- **코드 품질 관리** — 팀원들이 올린 코드를 검토하고 문제 없는지 확인
- **Merge 최종 결정** — `main` 브랜치에 병합 여부를 최종 판단
- **배포 모니터링** — GitHub Actions 배포 성공/실패 확인
- **충돌 해결** — 팀원 간 코드 충돌(Conflict) 발생 시 중재 및 해결
- **저장소 관리** — 브랜치 생성/삭제, GitHub Secrets 관리
...
**주요 책임:**
- **기능 개발** — 각자 담당 부분 코드 작성/수정
- **자기 브랜치 관리** — 내 브랜치에서만 작업, 주기적으로 Push
- **PR 작성** — 작업 완료 후 검토 요청서(PR) 작성
- **피드백 반영** — 리뷰 이후 요청 사항 수정
