<?php
/**
 * KBoard Calendar Skin - document.php
 * 교회일정 상세 페이지 (행사 날짜 표시 포함)
 */
?>
	...
	<div id="kboard-thumbnail-document">
		<div class="kboard-document-wrap" itemscope itemtype="http://schema.org/Article">
			...
			<div class="kboard-detail">
				<?php
				$ev_start = $content->option->event_date     ?? '';
				$ev_end   = $content->option->event_end_date ?? '';
				if ($ev_start):
					if ($ev_end && $ev_end !== $ev_start):
						$date_label = esc_html($ev_start) . ' ~ ' . esc_html($ev_end);
					else:
						$date_label = esc_html($ev_start);
					endif;
				?>
					<div class="detail-attr detail-event-date">
						<div class="detail-name">행사 날짜</div>
						<div class="detail-value calendar-event-date-badge">
							<?php echo $date_label; ?>
						</div>
					</div>
				<?php endif; ?>
				...
				<div class="detail-attr detail-date">
					...
					<div class="detail-value"><?php echo date('Y-m-d', strtotime($content->date)) ?></div>
				</div>
			...
			<div class="kboard-document-action">
				<?php if (!$board->meta->permission_vote_hide): ?>
					<div class="left">
						<button type="button" class="kboard-button-action kboard-button-like"
							onclick="kboard_document_like(this)" data-uid="<?php echo $content->uid ?>"
							title="<?php echo __('Like', 'kboard') ?>"><?php echo __('Like', 'kboard') ?> <span
								class="kboard-document-like-count"><?php echo intval($content->like) ?></span></button>
						<button type="button" class="kboard-button-action kboard-button-unlike"
							onclick="kboard_document_unlike(this)" data-uid="<?php echo $content->uid ?>"
							title="<?php echo __('Unlike', 'kboard') ?>"><?php echo __('Unlike', 'kboard') ?> <span
								class="kboard-document-unlike-count"><?php echo intval($content->unlike) ?></span></button>
					</div>
				<?php endif ?>
				<div class="right">
					<button type="button" class="kboard-button-action kboard-button-print"
						onclick="kboard_document_print('<?php echo $url->getDocumentPrint($content->uid) ?>')"
						title="<?php echo __('Print', 'kboard') ?>"><?php echo __('Print', 'kboard') ?></button>
				</div>
			</div>
			...
			<div class="kboard-prev-document">
				...
				if ($bottom_content_uid):
					...
					<a href="<?php echo esc_url($url->getDocumentURLWithUID($bottom_content_uid)) ?>"
						title="<?php echo esc_attr(wp_strip_all_tags($bottom_content->title)) ?>">
						<span class="navi-arrow">«</span>
						<span
							class="navi-document-title kboard-thumbnail-cut-strings"><?php echo wp_strip_all_tags($bottom_content->title) ?></span>
					</a>
				<?php endif ?>
			</div>
			<div class="kboard-next-document">
				...
				if ($top_content_uid):
					...
					<a href="<?php echo esc_url($url->getDocumentURLWithUID($top_content_uid)) ?>"
						title="<?php echo esc_attr(wp_strip_all_tags($top_content->title)) ?>">
						<span
							class="navi-document-title kboard-thumbnail-cut-strings"><?php echo wp_strip_all_tags($top_content->title) ?></span>
						<span class="navi-arrow">»</span>
					</a>
				<?php endif ?>
			</div>
		...
		<div class="kboard-control">
			<div class="left">
				...
				<?php if ($board->isReply() && !$content->notice): ?><a
						...
						class="kboard-thumbnail-button-small"><?php echo __('Reply', 'kboard') ?></a><?php endif ?>
			</div>
			<?php if ($content->isEditor() || $board->permission_write == 'all'): ?>
				<div class="right">
					<a href="<?php echo esc_url($url->getContentEditor($content->uid)) ?>"
						class="kboard-thumbnail-button-small"><?php echo __('Edit', 'kboard') ?></a>
					<a href="<?php echo esc_url($url->getContentRemove($content->uid)) ?>"
