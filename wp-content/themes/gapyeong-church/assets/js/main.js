...
// 테스트: 0이 아니면 이 페이지에서 라이브 상태를 처음 볼 때부터 지정 분 동안만 강제 ON AIR. 확인 후 반드시 0으로 되돌릴 것.
const GAPYEONG_LIVE_TEST_MINUTES = 10;
let _gapyeongLiveTestUntilMs = null;
...
function gapyeongIsScheduleOnAir(now = new Date()) {
    ...
    if (GAPYEONG_LIVE_TEST_MINUTES > 0) {
        if (_gapyeongLiveTestUntilMs === null) {
            _gapyeongLiveTestUntilMs = t + GAPYEONG_LIVE_TEST_MINUTES * 60 * 1000;
        }
        if (t < _gapyeongLiveTestUntilMs) {
            return true;
        }
    }
...
function checkLiveStatus() {
    ...
    if (!window._gapyeongLiveAirIntervalId) {
        const intervalMs = GAPYEONG_LIVE_TEST_MINUTES > 0 ? 15 * 1000 : 60 * 1000;
        window._gapyeongLiveAirIntervalId = window.setInterval(refreshLiveAirUi, intervalMs);
    }
}
